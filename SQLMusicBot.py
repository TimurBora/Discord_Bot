...
class SQLMusic:
    ...
    async def delete_row(self, guild_id, url):
        delete_row_query = '''DELETE FROM music_menu WHERE guild_id = ? AND music_url = ?'''

        await self.cursor.execute(delete_row_query, (guild_id, url))
        await self.sqlite_connection.commit()

    async def add_music(self, guild_id, music_url, music_name):
        create_insert_query = '''INSERT INTO music_menu VALUES(?, ?, ?, CURRENT_TIMESTAMP)'''
        
        await self.cursor.execute(create_insert_query, (guild_id, music_url, music_name))
        await self.sqlite_connection.commit()
    ...
    async def select_music_url(self, guild_id):
        create_select_query = '''SELECT music_url FROM music_menu WHERE guild_id = ? ORDER BY date_added'''
